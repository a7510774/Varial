...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSLog(@"SENDER DISPLAY NAME : %@", self.senderDisplayName);
...
{
    NSMutableDictionary *jsqMessageItem = [collectionView.dataSource collectionView:collectionView messageAtIndexPath:indexPath];
    
    if ([self isUserLeft:jsqMessageItem]) {
        return NO;
    }
...
- (BOOL)collectionView:(UICollectionView *)collectionView canPerformAction:(SEL)action forItemAtIndexPath:(NSIndexPath *)indexPath withSender:(id)sender
{
    ...
    //Custom code
    JSQMessagesCollectionView *collection = (JSQMessagesCollectionView *)collectionView;
    
    // If Message is teamstatus message should not show copy delete option
    NSMutableDictionary *jsqMessageItem = [self.collectionView.dataSource collectionView:collection messageAtIndexPath:indexPath];
    if ([self isUserLeft:jsqMessageItem]) {
        return NO;
    }
    
    id<JSQMessageData> messageItem = [self.collectionView.dataSource collectionView:collection messageDataForItemAtIndexPath:indexPath];
    if ([messageItem isMediaMessage] && action == @selector(copy:)) {
        return NO;
    }
    
    //
    if ([messageItem isMediaMessage] && action == @selector(forward:)) {
        if ([jsqMessageItem objectForKey:@"showForward"] != nil && [[jsqMessageItem objectForKey:@"showForward"] isEqualToString:@"NO"]) {
            return NO;
        }
    }
    ...
    if (action == @selector(copy:) || action == @selector(delete:) || (action == @selector(forward:) && [Util getBoolFromDefaults:@"is_chat_enabled"])) {
        return YES;
    }
    return NO;
}
...
- (void)collectionView:(JSQMessagesCollectionView *)collectionView performAction:(SEL)action forItemAtIndexPath:(NSIndexPath *)indexPath withSender:(id)sender
{
    if (action == @selector(copy:)) {
        ...
        [collectionView.dataSource collectionView:collectionView didForwardMessageAtIndexPath:indexPath];
    }
}
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"TYPING" object:nil userInfo:nil];
...
- (void)jsq_didReceiveMenuWillShowNotification:(NSNotification *)notification
{
    ...
    UIMenuItem* forwardMenu = [[UIMenuItem alloc] initWithTitle:NSLocalizedString(@"Forward", nil)  action:@selector(forward:)];
    menu.menuItems = [NSArray arrayWithObjects: forwardMenu,nil];
