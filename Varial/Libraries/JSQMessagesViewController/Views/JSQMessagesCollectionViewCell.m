...
- (void)awakeFromNib
{
    ...
    //Custom code
    self.uploadIndicator.layer.cornerRadius=self.uploadIndicator.bounds.size.height/2;
    self.uploadIndicator.layer.masksToBounds = YES;
    self.downloadView.layer.cornerRadius=self.downloadView.bounds.size.height/2;
    ...
    _downloadSpinner.lineWidth = 2.0f;
    _downloadSpinner.tintColor = UIColorFromHexCode(THEME_COLOR);
    
    self.uploadProgress.progressColor = UIColorFromHexCode(THEME_COLOR);
    self.uploadProgress.progressStrokeColor = UIColorFromHexCode(THEME_COLOR);
    self.uploadProgress.showValueString = FALSE;
    self.uploadProgress.progressLineWidth = 2;
    self.uploadProgress.maxValue = 1;
    self.uploadProgress.value = .05;
    self.uploadProgress.backgroundColor = [UIColor clearColor];
    self.uploadProgress.progressAngle = 100;
    
    self.downloadProgress.progressColor = UIColorFromHexCode(THEME_COLOR);
    self.downloadProgress.progressStrokeColor = UIColorFromHexCode(THEME_COLOR);
    self.downloadProgress.showValueString = FALSE;
    self.downloadProgress.value = .05;
    self.downloadProgress.backgroundColor = [UIColor clearColor];
}
...
- (BOOL)respondsToSelector:(SEL)aSelector
{
    if ([jsqMessagesCollectionViewCellActions containsObject:NSStringFromSelector(aSelector)]) {
        ...
        if([self isKindOfClass:[JSQMessagesCollectionViewCellOutgoing class]])
            x = self.messageBubbleContainerView.bounds.origin.x;
        else
            x = self.messageBubbleContainerView.bounds.origin.x+5;
        
        _coverView = [[UIView alloc] initWithFrame:CGRectMake(x, self.messageBubbleContainerView.bounds.origin.y, self.messageBubbleContainerView.bounds.size.width-5, self.messageBubbleContainerView.bounds.size.height)];
        _coverView.layer.cornerRadius = 17;
        _coverView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.4];
        [self.messageBubbleContainerView addSubview:_coverView];
        return YES;
    }
