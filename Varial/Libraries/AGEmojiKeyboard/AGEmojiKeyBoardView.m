...
- (NSMutableArray *)imagesForSegmentBar
{
    static NSMutableArray *array;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        array = [[NSMutableArray alloc] initWithObjects:@"emoji_tab_recent_icon.png",@"emoji_tab_face_icon.png",@"emoji_tab_bell_icon.png",@"emoji_tab_flower_icon.png",@"emoji_tab_car_icon.png",@"emoji_tab_symbols_icon.png", nil];
    });
    return array;
}
...
- (instancetype)initWithFrame:(CGRect)frame dataSource:(id<AGEmojiKeyboardViewDataSource>)dataSource {
  ...
  if (self) {
    ...
    self.segmentsBar = [[UISegmentedControl alloc] initWithItems:self.imagesForSegmentBar];
    self.segmentsBar.tintColor = UIColorFromHexCode(THEME_COLOR);
...
- (void)setSelectedCategoryImageInSegmentControl:(UISegmentedControl *)segmentsBar
  ...
  for (int i=0; i < self.segmentsBar.numberOfSegments; ++i) {
    [segmentsBar setImage:[UIImage imageNamed:self.imagesForSegmentBar[i]] forSegmentAtIndex:i];
  }
  [segmentsBar setImage:[UIImage imageNamed:self.imagesForSegmentBar[index]] forSegmentAtIndex:index];
}
