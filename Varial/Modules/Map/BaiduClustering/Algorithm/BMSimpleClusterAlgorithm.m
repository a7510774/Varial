...
@implementation BMSimpleClusterAlgorithm {
...
- (NSArray<id<BMCluster>> *)clustersAtZoom:(float)zoom {
  NSMutableArray<id<BMCluster>> *clusters =
      [[NSMutableArray alloc] initWithCapacity:kClusterCount];

  for (int i = 0; i < kClusterCount; ++i) {
    if (i >= _items.count) break;
    id<BMClusterItem> item = _items[i];
    [clusters addObject:[[BMStaticCluster alloc] initWithPosition:item.position]];
  }

  NSUInteger clusterIndex = 0;
  for (int i = kClusterCount; i < _items.count; ++i) {
    id<BMClusterItem> item = _items[i];
    BMStaticCluster *cluster = clusters[clusterIndex % kClusterCount];
