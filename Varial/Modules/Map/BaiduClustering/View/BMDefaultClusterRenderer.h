...
//@class GMSMarker;
@class BMKMapView;
@class BMClusterAnnotation;
@class BMKAnnotationView;

@protocol BMCluster;
@protocol BMClusterIconGenerator;
@protocol BMClusterRenderer;

/**
 * Delegate for id<BMClusterRenderer> to provide extra functionality to the default
 * renderer.
 */
@protocol BMClusterRendererDelegate<NSObject>

@optional

/**
 * Returns a marker for an |object|. The |object| can be either an id<BMCluster>
 * or an id<BMClusterItem>. Use this delegate to control of the life cycle of
 ...
 * item marker.
 */
- (void)renderer:(id<BMClusterRenderer>)renderer didRenderAnnotationView:(BMKAnnotationView *)view;
 ...
 * region are shown.
 */
@interface BMDefaultClusterRenderer : NSObject<BMClusterRenderer>
...
@property(nonatomic, nullable, weak) id<BMClusterRendererDelegate> delegate;
