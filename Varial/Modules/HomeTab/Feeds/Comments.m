...
- (void)viewDidLoad {
    ...
    UIMenuItem* deleteMenu = [[UIMenuItem alloc] initWithTitle:NSLocalizedString(@"Delete", nil)  action:@selector(deleteComment:)];
...
-(void) getRecentComments:(NSNotification *) data{
    ...
    if (!isPostExpired) {
        ...
        if ([commentsList count] == 0) {
            ...
            NSString *url = COMMENT_LIST_FOR_POST;
            
            if (_mediaId != nil) {
                [inputParams setValue:_mediaId  forKey:@"media_id"];
                url = COMMENT_LIST_FOR_MEDIA;
            }
...
//    [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleLightContent];
    [self getUserDetailsFromSession];
...
-(void)getUserDetailsFromSession
{
...
- (void)createTheLocalComment{
    ...
    [comment setValue:name forKey:@"player_name"];
    ...
    [comment setValue:[Util getFromDefaults:@"player_id"] forKey:@"comment_owner_id"];
...
-(BOOL)inputValidation
{
    ...
    NSLog(@"MSG Length %lu",(unsigned long)message.length);
    if ([[message stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] length] == 0)
    {
        return FALSE;
    }
    if ([[message stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] length] > POST_CONTENT_MAX)
    {
...
-(void)getCommentsOldList
{
    ...
    if ([commentsList count] > 0) {
        ...
        lastCommentId = [comment valueForKey:@"comment_id"];
    }else{
        lastCommentId = @"";
    }
    ...
    [inputParams setValue:_postId  forKey:@"post_id"];
    [inputParams setValue:lastCommentId  forKey:@"comment_id"];
    ...
    [[Util sharedInstance]  sendHTTPPostRequest:inputParams withRequestUrl:url withCallBack:^(NSDictionary * response){
        if([[response valueForKey:@"status"] boolValue]){
            mediaBase = [response valueForKey:@"media_base_url"];
            if (_mediaId != nil) {
                [self createCommentsList:[[response objectForKey:@"feed_comment_list_media"] mutableCopy] isBottom:TRUE];
            }
            else{
                [self createCommentsList:[[response objectForKey:@"feed_list"] mutableCopy] isBottom:TRUE];
            }
        }
        else
        {
...
-(void)createCommentsList:(NSMutableArray *)list isBottom:(BOOL)isBottom{
    ...
    for (int i = 0 ; i < [list count]; i++){
        ...
        if ([localComments indexOfObject:serverCommentId] == NSNotFound ) {
            ...
            NSString *profileUrl = [NSString stringWithFormat:@"%@%@",mediaBase,[comment valueForKey:@"profile_image"]];
...
{
    if([_isFromFeedsPage isEqualToString:@"YES"])
    {
        // Update local storage
        ViewController *viewController = [[self.navigationController viewControllers] firstObject];
        if (_feeds != nil && [_feeds count] != 0) {
            [LocalStorageManager localStorage:@"FEED" Response:_feeds feedType:[viewController.selectedFeedType intValue]];
        }
    }
}
...
- (void)ShowMenu:(UITapGestureRecognizer *)tapRecognizer
{
    ...
    if([commentsList count] > indexPath.row)
    {
        //Show menu
...
-(void)commentDelete
{
    if([commentsList count] > menuIndex)
    {
        ...
        NSString *URL = DELETE_COMMENT_FOR_POST;
        if (_mediaId != nil) {
            URL = DELETE_COMMENT_FOR_MEDIA;
            [inputParams setValue:_mediaId  forKey:@"media_id"];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    lblComments.enabledTextCheckingTypes = NSTextCheckingTypeLink;
    ...
    if([commentsList count] > indexPath.row)
    {
        ...
        lblName.text = [comment valueForKey:@"player_name"];
        //lblDateTime.text = [Util timeAgo:[comment valueForKey:@"time_zone"]];
